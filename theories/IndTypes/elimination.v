...
(*From IndTypes*) Require Export MPSTSR.IndTypes.syntax. 
...
| GBranch a gs => (~~ (ptcp_from a == ptcp_to a)) && all (action_pred \o snd) gs
...
| GBranch a gs => (0 < size gs) && all (size_pred \o snd) gs
...
| EBranch _ _ es => (0 < size es) && all (esize_pred \o snd) es
...
| GBranch  _ gs => all (gcontractive \o snd) gs
...
| EBranch _ _ gs => all (lcontractive \o snd) gs
...
| GBranch _ es => foldr (fun e0 acc => (gsize e0.2) + acc ) 2 es
...
| EBranch _ _ es => foldr (fun e0 acc => (esize e0.2) + acc ) 2 es
| ERec  e0 => (esize e0).+1
| _ => 1
end.
...
Variables (Pg : gType -> Type) 
          ...
          (Pe : lType -> Type) 
          (Pm : mysort -> Type)
          (P_glist : seq (nat * gType) -> Type)
          (P_elist : seq (nat * lType) -> Type)
...
Hypothesis Pg_branch : (forall (a : action) (l : seq (nat  * gType)), P_glist l  -> Pg (GBranch a l)).

Hypothesis Pv_vst : forall m : mysort, Pm m -> Pv (VSort m).
...
Hypothesis Pe_branch : forall (d : dir) (c : ch) (l : seq (nat * lType)), P_elist l  -> Pe (EBranch d c l).
Hypothesis Pe_rec : forall e : lType, Pe e -> Pe (ERec e).

Hypothesis P_glist_0 : P_glist nil.
Hypothesis P_glist_cons : forall n g, Pg g -> forall l, P_glist l -> P_glist ((n,g)::l).

Hypothesis P_elist_0 : P_elist nil.
Hypothesis P_elist_cons : forall n e, Pe e -> forall l, P_elist l -> P_elist ((n,e)::l).
...
Fixpoint gType_rect g : Pg g :=
  let fix seq_gType_rect gs : P_glist gs := 
    match gs with 
     | [::] => P_glist_0
     | (n,g')::gs' => P_glist_cons n (gType_rect g') (seq_gType_rect gs') 
     end in
  match g with 
   | GVar n => Pg_var n
   | GEnd => Pg_end
...
in
match v with 
| VSort ss => Pv_vst (mysort_rect ss)
...
let fix seq_lType_rect ss : P_elist ss :=
 match ss with
  | [::] => P_elist_0
  | (n,m)::ms => P_elist_cons n (lType_rect m) (seq_lType_rect ms)
 end in
match e with
| EVar n => Pe_var n
| EEnd => Pe_end
...
(*Fix r, to convince guardedness checker in gType_eqb of termination*)
Definition list_eq2 {A} (r : A -> A -> bool) (l0 l1 : seq (nat * A)) :=  
(fix list_eq l0 l1 :=
match l0,l1 with
| nil,nil => true 
| (n,x1)::s1',(n',x2)::s2' => (n == n') && (r x1 x2) && (list_eq s1' s2')
| _,_ => false 
end) l0 l1.
...
| GEnd, GEnd => true
| GBranch a gs, GBranch a' gs' => (a == a') && (list_eq2 gType_eqb gs gs') 
| GVar n, GVar n' => n == n'
| _, _ => false
end
with value_eqb (v0 v1 : value) {struct v0} :=
match v0, v1 with 
| VSort s0, VSort s1 => mysort_eqb s0 s1
...
| EBranch d0 ch0 s0, EBranch d1 ch1 s1 => ((d0,ch0) == (d1,ch1)) && (list_eq2 lType_eqb s0 s1)
...
Definition mut_ind := (@mut_ind_aux Pg Pv Pe Ps (ForallT (Pg \o snd)) (ForallT (Pe \o snd)) (ForallT Ps)).  
...
ssa.  
elim : l X. done. simpl. ssa. destruct a0. ssa. inv X. inv X. auto. ssa. 
elim : l X. done. simpl. ssa. destruct a. ssa. inv X. inv X. auto. 
Qed.
...
- destruct b;try done. ssa. rewrite (eqP H0). f_equal. 
  elim : l l0 H1 X. case. done. done. simpl. intros. destruct l0;try done. destruct a1;done.
(*  ssa.  f_equal. inv X. rewrite <- H4.   done. done. inv X.  auto. *)
- destruct a1;try done. destruct p;try done. ssa. rewrite (eqP H1). f_equal. f_equal. inv X.  auto. 
  apply/H.  done. inv X. destruct b. ssa. erewrite H. eauto. done. done. 
...
- destruct b;try done. ssa. move : H0. move/eqP. case. intros. subst. 
  ...
  destruct a.   ssa.  f_equal. destruct a.  destruct p. ssa. 
  rewrite (eqP H1). f_equal. inv X. ssa. 
  destruct a. destruct p. ssa. apply/H. inv X. done. 
...
Definition true_pred A := fun (_ : A) => true.
...
Definition gType_rect_true := (@gType_rect Pg  (@true_pred value) (@true_pred lType) (@true_pred mysort) (fun gl => forall n g, (n,g) \in  gl ->Pg g)
                                                (@true_pred (seq (nat * lType)))
                                                (@true_pred (seq mysort))).

Definition lType_rect_true := (@lType_rect (@true_pred gType) (@true_pred value) Pe (@true_pred mysort) (@true_pred (seq (nat * gType)))
                                                (fun el => forall n e,  (n,e) \in el -> Pe e)
                                                (@true_pred (seq mysort))).
...
Lemma gType_ind
     : forall (Pg : gType -> Prop  ),
       ...
       (forall (a : action) (v : value),
        forall g : gType,
        Pg g -> Pg (GMsg a v g)) ->
       (forall (a : action) (l : seq (nat * gType)),
         (forall n g, (n,g) \in l -> Pg g) -> Pg (GBranch a l)) ->
       forall g : gType, Pg g.
Proof.
intros.  apply : gType_rect_true;auto. done. 
intros. rewrite inE in H6. destruct (orP H6). move/eqP : H7. case. 
intros. subst. done. eauto. 
Qed.
...
Lemma lType_ind
     : forall (Pe : lType -> Prop),
       ...
       (forall d c (l : seq (nat * lType)),
         (forall n g, (n,g) \in l -> Pe g) -> Pe (EBranch d c l)) ->
       forall g : lType, Pe g.
Proof.
intros. apply : lType_rect_true;auto.
all: try (rewrite /true_pred; constructor). 
intros.  done. intros. rewrite inE in H6. destruct (orP H6). move/eqP : H7. case. intros. 
subst. done. eauto. 
Qed.
