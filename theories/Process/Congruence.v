...
Inductive Cong : process -> process -> Prop := 
 ...
 | CRes P P' : Cong P P' -> Cong (ResCh P) (ResCh P')
 | C2V P P' : Cong P P' -> Cong (ResVal P) (ResVal P').
...
Proof. 
intros. move : H=>Hvar. elim : H0 Ms Hvar E Q C Hun Hpred HunQ ;intros. 
split. 
...
- split. intros. inv H1. econ. eauto. eapply H0;eauto.
  rewrite dom_insertL1. rewrite cat_uniq. ssa.

  rewrite map_inj_in_uniq.
  inv H3. ssa. inv H5. inv H2. ssa. 
  subst. rewrite /makeLs -map_comp. 
  rewrite map_inj_in_uniq //.
  intro. ssa. inv H12.
  intro. ssa.  inv H5.
  inv H3. ssa. inv H10.
  inv H7. ssa.
  de x. de y. subst.
  apply uniq_in_pair in H2.
  apply uniq_in_pair in H4.
  rewrite H4 in H2. inv H2.
  rewrite /dom /makeLs -!map_comp.
  rewrite map_inj_in_uniq. done.
  intro. ssa. inv H15.
  rewrite /dom /makeLs -!map_comp. 
  rewrite map_inj_in_uniq. done. 
  intro. ssa. inv H15.
  apply/hasPn. intro. move/mapP. case. ssa.
  subst. apply/negP. intro. de (mapP H2). de x0. asimpl in H5. de s.
  rewrite map_inj_in_uniq. done. intro. ssa.

  rewrite dom_insertQ. rewrite cat_uniq. ssa.

  rewrite map_inj_in_uniq.
  inv H3. ssa. inv H5. inv H2. ssa. 
  subst. rewrite /makeQs /dom -!map_comp. 
  rewrite map_inj_in_uniq //.
  intro. ssa. inv H12.
  intro. ssa.  inv H5.
  apply/hasPn. intro. 
  rewrite dom_map_keys.
  move/mapP. case. ssa. subst.
  apply/negP. intro. de (mapP H2). de x0. asimpl in H5. de s.
  rewrite dom_map_keys.
  rewrite map_inj_in_uniq. done. intro. ssa.

- intros. inv H1. econ. eauto. eapply H0;eauto.
  rewrite dom_insertL1. rewrite cat_uniq. ssa.
...
- split. intros. inv H1. econ. eauto. eapply H0;ssa.
  move: Hvar. rewrite /only_vars. rewrite dom_insertS1 /=.
  rewrite all_map. intro. apply/allP. intro. ssa.
  de x. apply (allP Hvar) in H2. done.

  intros. inv H1. econ. eauto. eapply H0;ssa.
  move: Hvar. rewrite /only_vars. rewrite dom_insertS1 /=.
  rewrite all_map. intro. apply/allP. intro. ssa.
  de x. apply (allP Hvar) in H2. done.
Qed.
