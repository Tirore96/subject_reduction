...
(*Inductive redexType := Value | SessCh | Label.*)
(* Definition plabel := option (dir * schl * ch). *)



(* Inductive SemL : defs -> process -> plabel -> process -> Prop := *)
(*  | SLink D q P : CanLink q P -> SemL D P None (ResCh (Par (reduce_link P) (make_queues (make_cs q.+1))))  *)
(*  | SSend D e v s i p P msgs: EvalE e v -> SemL D (Par (ValSend ((schlT s p)) i e P) (MsgQ (schliT s i) msgs)) (Some (Sd,s,i)) *)
(*                                              (Par P (MsgQ (schliT s i) (msgs ++ [::(msgT (valM v) p)]))) *)
(*  | SDel D s s' i p p' P msgs: SemL D (Par (SessDel ( (schlT s p)) i (schlT s' p') P) (MsgQ (schliT s i) msgs)) (Some (Sd,s,i)) *)
(*                                (Par P (MsgQ (schliT s i) (msgs++[::msgT (schM (schlT s' p')) p])))  *)
(*  | SLabel D s l i p P msgs: SemL D (Par (LabelSel (( (schlT s p))) i l P) (MsgQ (schliT s i) msgs)) (Some (Sd,s,i)) *)
(*                                 (Par P (MsgQ (schliT s i) (msgs++[::msgT (labelM l) p]))) *)

(*  | SRec D v s i p p' P msgs: SemL D (Par (ValRec ( (schlT s p)) i P)  *)
(*                                      (MsgQ (schliT s i) ((msgT (valM v) p')::msgs))) (Some (Rd,s,i)) *)
(*                               (Par (my_subst_valBool v P)  *)
(*                                    (MsgQ (schliT s i) (msgs))) *)
(*  | SSessRec D s s' i p p' p'' P msgs: SemL D (Par (SessRec ( (schlT s p)) i P) (MsgQ (schliT s i) ((msgT (schM (schlT s' p')) p'')::msgs))) (Some (Rd,s,i)) *)
(*                                    (Par (my_subst_schl s' p' P) (MsgQ (schliT s i) (msgs))) *)
(*  | SBranch D s l i p p' P Ps msgs : lookup Ps l = Some P -> *)
(*                                           SemL D (Par (LabelBr ( (schlT s p)) i Ps) (MsgQ (schliT s i) ((msgT (labelM l) p')::msgs))) (Some (Rd,s,i)) *)
(*                                                 (Par P (MsgQ (schliT s i) (msgs))) *)
(*  | STrue D e P0 P1 : EvalE e (boolB true) -> SemL D (If e P0 P1) None P0 *)
(*  | SFalse D e P0 P1 : EvalE e (boolB false) -> SemL D (If e P0 P1) None P1 *)
(*  | SCall D defi e v ss : EvalE e v -> defi < size D -> SemL D (PCall defi e ss) None *)
(*                                            (subst_process ids (v .: ids) ids (fun n => nth ( (schlT (var_schl 0) (Ptcp 0))) ss n) (nth Inact D defi)) (*Discuss this change from original communication safety*) *)
(*  | SScopCh D P P' pl : SemL D P pl P' -> SemL D (ResCh P) pl (ResCh P') *)
(*  | SScopName D P P' pl : SemL D P pl P' -> SemL D (ResVal P) pl (ResVal P') *)
(*  | SPar D P Q P' pl : SemL D P pl P' -> SemL D (Par P Q) pl (Par P' Q) *)
(*  | SStr D P P' Q' Q pl : Cong P P' -> SemL D P' pl Q' -> Cong Q' Q -> SemL D P pl Q. *)
(* Hint Constructors Sem SemL. *)
(* Lemma SemToL : forall D P P', Sem D P P' -> exists pl, SemL D P pl P'. *)
(* Proof. *)
(* move=> D P P'. elim;ssa;eauto. *)
(* exists None. con. done. *)
(* Qed. *)

(* Lemma SemFromL : forall D P pl P', SemL D P pl P' -> Sem D P P'. *)
(* Proof. *)
(* move=> D P pl P'. elim;ssa;eauto. *)
(* con. done. *)
(* Qed. *)

(* Lemma subject_reduction_full_label : forall D P P' Ms Ds E Q C pl, SemL D P pl P' -> side_conds2 Ds D Ms P E Q ->  OFT Ms Ds P E Q C ->   *)
(* exists E' E'' Q', LQ_red E Q E' Q' /\ EQ_same E' E'' /\ OFT Ms Ds P' E'' Q' C /\ side_conds2 Ds D Ms P' E'' Q'. *)
(* Proof.  *)
(* intros. apply SemFromL in H. *)
(* apply/subject_reduction_full;eauto. *)
(* Qed. *)
...
(*Lemma to_weak_coherent_as : forall E Q, weak_coherent E Q -> exists L, weak_coherent_as E Q L.
Proof.
intros. inv H.
Qed.
...
(*Lemma lookup_fun : forall (A B C : eqType) (E : seq (A * B)) (f : B -> C) (a : A),  E -> uniq (dom E) ->
 lookup (map E) a =  
*)
