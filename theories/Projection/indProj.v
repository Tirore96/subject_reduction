...
Require Import MPSTSR.Projection.projectDecide.
...
| GBranch a gs => if comp_dir p a is Some d then EBranch d (action_ch a) (map (prod_map id (trans p)) gs) 
                                           else head EEnd (map ((trans p) \o snd) gs)
...
destruct l=>//=. destruct p0.   ssa. apply/H.  eauto. 
...
destruct l;try done. ssa. destruct x0. ssa. eauto. destruct l.  ssa. ssa. destruct p0. ssa. eauto. 
Qed.
...
move : q0. rewrite /comp.  destruct x0. ssa. eapply H in p0 as p0'. 2 : eauto. 
exists (gType_fv s0)=>//=. have : s0 = snd (s,s0). done. move=>->. apply/map_f. done. 
destruct l;try done. 
ssa. rewrite mem_cat. apply/orP. left. destruct p0.  ssa. apply/H=>//=. eauto. 
Qed.
...
f_equal. apply/eq_in_map=> x xIn. simpl. destruct x.  ssa. f_equal. eauto.  
rewrite -!map_comp. destruct l. done. simpl. destruct p0.  ssa. eauto. 
Qed. 
...
ssa. rewrite /prod_map. destruct a0.  ssa. destruct a0. pc. ssa. 
...
pfold. con. rewrite !full_eunf_idemp. punfold H4.  inv H4. done. 
...
con;eauto. ssa. rewrite H0 H4 //=. pc. eauto.
...
rewrite etocoind_full_eunf. rewrite (etocoind_full_eunf e1). inv H;seq. pfold. con. pc. eauto.
...
- rewrite etocoind_full_eunf. rewrite proj_eq_full H0 /=.  destruct (comp_dir p a) eqn:Heqn;try done. cbn. 
  ...
  inv H8. inv H11. pclearbot. simpl in *. con;eauto. ssa. pc. rewrite /prod_map. destruct a0. ssa.
  rewrite /id. subst. rewrite -H0 //=. pc. rewrite /prod_map. destruct a0. ssa. eauto. 
  have : ~ comp_dir p a by eauto. comp_disc. 
...
- rewrite etocoind_full_eunf. rewrite proj_eq_full H2 /=.  destruct (comp_dir p a) eqn:Heqn;try done.
  ...
  rewrite -etocoind_full_eunf. destruct ecs;try done. inv H10.  pclearbot. simpl in *. ssa.  pc.
  apply/H1. simpl. eauto. apply/H0.  simpl. eauto. 2 : eauto. destruct p1. ssa. 
  have : (n,g1) = (n,g1) \/ In (n,g1) ecs. eauto. move/H13. ssa. pclearbot. done. 
  ...
  simpl in *. destruct H13;try done. 
  apply/H1. simpl. eauto. apply/H0.  simpl. eauto. 2 : eauto. pfold. con. 
  intro. apply/H6. econstructor 4.  simpl. eauto. done. punfold H7. inv H7. injt. inv H15. pc. done. 
...
destruct H9. ssa. subst. con=>//=.  inv H4. rewrite H5 //=. 
...
con;eauto. ssa. rewrite H0 H2 //=. pc.  eauto. 
done. 
