...
Require Export MPSTSR.CoTypes.coGlobal MPSTSR.CoTypes.coLocal.
...
| po_branch2 a g (gs : seq (nat * gcType)) : In g gs -> part_of p g.2 ->   part_of p (GCBranch a gs).
...
Lemma part_of_ind2
     : forall (p : ptcp) (P : gcType -> Prop),
       ...
       (forall (a : action) (gs : coseq (nat * gcType)), comp_dir p a -> P (GCBranch a gs)) ->
       (forall (a : action) g  (gs : seq (nat * gcType)), ~ comp_dir p a -> In g gs -> part_of p g.2 -> P g.2 -> P (GCBranch a gs)) ->
...
Lemma not_part_branch : forall p a g (gs : seq (nat * gcType)), ~ part_of p (GCBranch a gs) -> In g gs -> ~ part_of p g.2.
Proof.  
intros. intro.  apply/H. eauto. 
Qed.

Lemma not_part_branch2 : forall p a (gs : seq (nat * gcType)), ~ part_of p (GCBranch a gs) -> ~ comp_dir p a.  
Proof.  
intros. intro.  apply/H. eauto. 
Qed.
...
| poac_branch2 a (gs : seq (nat * gcType))  :  (forall g, In g gs -> part_of_all p g.2) ->  part_of_all p (GCBranch a gs). (*May not be vacously true, require their to be at least one element in g, now part_of_all -> part_of and also part_of_all and ~ part_of cannot coincide*)
...
Lemma part_of_all_ind2
     : forall (p : ptcp) (P : gcType -> Prop),
       ...
       (forall (a : action) (gs : coseq (nat * gcType)), comp_dir p a -> P (GCBranch a gs)) ->
       (forall (a : action) (gs : seq (nat * gcType)),  ~ comp_dir p a ->  (forall g, In g gs -> part_of_all p g.2)   ->  (forall g, In g gs -> part_of_all p g.2 -> P g.2) ->  P (GCBranch a gs)) ->
...
Variant FinSeq_gen (R : gcType ->  Prop) : gcType   -> Prop :=
 ...
 | fin_seq_branch (es : seq (nat * gcType))  d :  Forall (R \o snd) es -> FinSeq_gen R (GCBranch d es) 
...
intros. move/List.Forall_forall : H. intros.  cbn. apply/LE/H.  eauto. 
con. 
Qed.
...
Lemma Forall_snd : forall {A B : Type} (l : seq (A * B)) (P : B -> Prop) , Forall (P \o snd) l -> Forall P (map snd l).  
Proof. move => A B. elim. ssa. ssa. 
con. inv H0 . inv H0. eauto.
Qed. 

Lemma Finite_branch : forall a g (gs : seq (nat * gcType)),  Finite (GCBranch a gs) -> In g gs -> Finite g.2. 
Proof. 
...
Lemma Finite_coerce : forall a gs, Finite (GCBranch a gs) -> exists (gs' : seq (nat * gcType)), gs = gs'. 
Proof. 
...
Inductive cproject_gen (p : ptcp) (R : gcType ->  lcType -> Prop) : gcType -> lcType -> Prop :=
 ...
 | cproject_gen_branch_f (gs : seq (nat * gcType)) (es : seq (nat * lcType)) a d :  comp_dir p a = Some d -> size gs = size es ->
                                        (forall p, In p (zip gs es) ->  p.1.1 = p.2.1 /\ R p.1.2 p.2.2 ) -> cproject_gen p R (GCBranch a gs) (ECBranch d (action_ch a) es)
 | cproject_gen_branch_o g (gs : seq (nat * gcType))  a e : comp_dir p a = None -> In g gs ->  (*We need list to be non -empty otherweise it projects to anything*)
                                    (forall g', In g' gs ->  part_of_all p g'.2 /\  R g'.2 e) ->  cproject_gen p R (GCBranch a gs) e
...
intros. ssa. move/H1 : H2.  ssa. apply/LE. move/H1 : H2. ssa. econstructor;eauto. econ. 
move/H1 : H2. ssa. move/H1 : H2. ssa. con. all : eauto.
Qed.
