...
Definition unf g := if g is GRec g' then g' [g g .: GVar]  else g.
...
Lemma guarded_test : forall e sigma i,  ~~ guarded i e -> iter (mu_height e) unf e [g sigma ] = sigma i. 
Proof. 
elim;rewrite //=;intros. 
...
CoInductive gcType :=
 ...
 | GCBranch : action -> coseq (nat * gcType) -> gcType.
...
| GBranch a gs => GCBranch a (comap (liftL f) (to_coseq gs))
| _  => GCEnd
end.
...
| GBranch a gs => GCBranch a (map (liftL gtocoind) gs)
| _  => GCEnd
end.
Proof. 
...
Inductive gUnravel_gen (R : gType -> gcType  -> Prop) : gType -> gcType  -> Prop :=
 ...
 | gUnravel_gen_branch (gs : seq (nat * gType)) (gcs : seq (nat * gcType)) a : size gs = size gcs -> Forall (fun p => p.1.1 = p.2.1 /\ R p.1.2 p.2.2) (zip gs gcs) -> gUnravel_gen R (GBranch a gs) (GCBranch a gcs) (*restrict gcType to be an inductive list in disguse, only coerced from inductive to coinductive to let gtocoind pass productivity test of Coq*)
...
move/List.Forall_forall : H0. eauto. con. move/H0 : H1. ssa. move/H0 : H1. ssa. con. done. con.
Qed. 
...
Variant gUnravel2_gen (R : gType -> gcType  -> Prop) : gType -> gcType  -> Prop :=
 ...
 | gUnravel2_gen_branch (es : seq (nat * gType)) ( ecs : seq (nat * gcType))  d : size es = size ecs ->  Forall (fun p => p.1.1 = p.2.1 /\ R p.1.2 p.2.2) (zip es ecs) -> gUnravel2_gen R (GBranch d es)  (GCBranch d ecs)
 | gUnravel2_gen_end :   gUnravel2_gen R GEnd  GCEnd.

Lemma gUnravel2_gen_mon : monotone2 gUnravel2_gen. 
Proof. move => x0 x1. intros. induction IN;try done. con;eauto. 
con;eauto. apply/List.Forall_forall. intros. 
move/List.Forall_forall : H0. eauto. con. move/H0 : H1. ssa. move/H0 : H1. ssa. con. 
Qed. 
...
Variant gInvPred_gen (R : gType ->  Prop) : gType   -> Prop :=
 ...
 | rol_gen_branch (es : seq (nat * gType))  d :  Forall (R \o snd) es -> gInvPred_gen R (GBranch d es) 
 | rol_gen_end :   gInvPred_gen R GEnd .

Lemma gInvPred_gen_mon : monotone1 gInvPred_gen. 
Proof. move => x0 x1. intros. induction IN;try done. con;eauto. 
con;eauto. apply/List.Forall_forall. intros. cbn.
move/List.Forall_forall : H. intros.  apply/LE. apply/H. done. 
con. 
Qed. 
...
con. simpl in *. ssa.  pclearbot.   right. apply/CIH. eauto. ssa. apply/IH.  eauto. done. 
con. 
Qed.
...
simpl. intros. inv H2. pclearbot. con. ssa. eauto.
con. 
Qed.
...
induction H0;auto. constructor. ssa.  pclearbot. eauto. eauto.  
pfold. constructor. rewrite full_unf_subst. 
punfold IHgUnravel_gen. inv IHgUnravel_gen. 
...
induction H3;auto. ssa. pclearbot. con.  eauto. eauto.
...
con. inv H2.  pclearbot. ssa. right. eauto. inv H2. ssa. pclearbot. eauto. eapply IH. eauto. inv H2.  
Qed. 
...
| GBranch d es => flatten (map (enumg \o snd) es)
| GEnd => nil
| GVar n => nil
end.
...
- rewrite inE in H0. destruct (orP H0). rewrite (eqP H1) inE eqxx //=. 
  ...
  destruct (mapP H2). subst. destruct x0.  apply/H. ssa. eauto. done. 
Qed.
...
Proof. 
destruct e;try done. ssa. elim : l sigma. ssa. ssa. f_equal. rewrite /prod_map. destruct a0.  done. done. 
Qed. 
...
apply/flattenP. exists (enumg a').
move/nthP : H1. move/(_ GEnd). case. ssa. subst. 
apply/mapP.  exists (nth (0,GEnd) l x). apply/mem_nth. rewrite size_map in p. done. cbn. 
erewrite nth_map. eauto. rewrite size_map in p. done. 
apply/selfe. 
rewrite inE. apply/orP. right. destruct (flattenP H2). 
destruct (mapP H3). subst. apply/flattenP. 
exists ((enumg x0.2)). apply/map_f. done.
destruct x0. ssa. 
apply/H. eauto. eauto. done. 
Qed.
...
rewrite -!map_comp. 
rewrite /comp. 
have : map (fun x => x [g GRec g .: GVar] [g sigma]) (enumg g) =  map (fun x =>  x [g GRec g [g GVar 0 .: sigma >> ⟨g ↑ ⟩] .: sigma]) (enumg g). 
...
destruct (mapP H3). subst. apply/flattenP.  exists ([seq e0 [g sigma] | e0 <- enumg g]). 
have : map
          (fun x0 : fin * gType => map (subst_gType sigma) (enumg x0.2)) l =
map
          (fun x0 : gType => map (subst_gType sigma) (enumg x0)) (map snd l).
clear H H2 H1 H4. 
elim : l. ssa. ssa. f_equal. done. 
move=>->. apply/map_f. 
move/nthP : H4. move/(_ (0,GEnd)). case. intros. subst. 
apply/nthP. exists x0. rewrite size_map //=. erewrite nth_map.  
instantiate (1 := (0,GEnd)). rewrite q. ssa. done. apply/map_f. done. 
...
(*
Lemma enumg_subst2 : forall e e' sigma (S : seq gType), (forall n e0,  e0 \in enumg (sigma n) -> e0 \in S) -> e' \in enumg e [g sigma] -> e' \in map (fun e0 =>  e0 [g sigma]) (enumg e) \/ e' \in S. 
Proof. 
elim;rewrite //=;intros. 
destruct (is_gvar (sigma n)) eqn:Heqn. destruct (sigma n);try done. simpl in H0. rewrite inE in H0. rewrite (eqP H0). 
auto. right. apply/H. lia. 
rewrite inE in H0. rewrite (eqP H0). auto.
rewrite inE in H1.  destruct (orP H1). 
rewrite (eqP H2). rewrite inE. left. rewrite eqxx. lia. 
rewrite -!map_comp. 
...
destruct p. destruct (mapP H2). subst. asimpl. left. rewrite inE. apply/orP. right. apply/map_f. done. 
right. 
2 : { intros. instantiate (1 := GVar 0 ::  map (fun e0 => e0 ⟨g succn ⟩) S).
move : H2. destruct n. asimpl. ssa. 
asimpl. rewrite enumg_ren. intros. rewrite inE. apply/orP. right.
  destruct (mapP H2). subst. apply H0 in H3. 
apply/map_f.  done. } 

rewrite inE in H2. de (orP H2). norm_eqs. simpl.  
admit.

destruct (mapP H3). subst. asimpl. done.
rewrite inE in H1.  destruct (orP H1). 
rewrite (eqP H2). auto.
rewrite inE. intros. 
eapply H in H2. destruct H2.  left. apply/orP. right. done. eauto. 
...
Lemma map_snd : forall {A B C : Type} (l : seq (A * B)) (f : B -> C), map (f \o snd) l = map f (map snd l).
Proof. 
move => A B C. elim. ssa. ssa. 
destruct a. ssa. f_equal. done. 
Qed. 
...
have : forall e0 : gType,
      In e0 (map snd l) ->  upaco2 (ApplyF full_unf ssrfun.id \o gUnravel2_gen) R e0 (gtocoind e0). 
...
instantiate (1 := co_nth GCEnd (map snd ecs) x).
apply index_ForallIC. 
apply coerce_forall2. rewrite !size_map.  done. clear H0 H1 H2 p.
elim : es ecs H3 H4. case=>//=. ssa. ssa. 
destruct ecs. ssa. ssa. con. ssa. inv H4.  ssa. pclearbot. done. inv H4. ssa. done. 
Qed. 
...
Inductive gInvPred2_gen (R : gType ->  Prop) : gType   -> Prop :=
 ...
 | rol2_gen_branch (es : seq (nat * gType))  d :  Forall (R \o snd) es -> gInvPred2_gen R (GBranch d es) 
...
move/List.Forall_forall : H. intros.  apply/LE/H=>//=. 
con. con. 
Qed. 
...
Fixpoint gType_fv e :=
  ...
  | GBranch _ gs => flatten (map (gType_fv \o snd) gs)
...
f_equal. apply/eq_in_map=> x xIn. simpl. 
destruct x. 
ssa. eauto. 
Qed.
...
rewrite flatten_cat. f_equal. 
destruct a0. 
ssa. eauto. auto. apply/H. eauto. 
Qed.
...
Inductive has_var (n : nat ) :  gType -> Prop := 
 ...
 | hv2 l g gs a : In (l,g) gs -> has_var n g  -> has_var n (GBranch a gs)
...
con. apply/H0. done. simpl. econstructor. apply/inP. apply/mapP. exists (l,g0). apply/inP. done. rewrite /prod_map.  eauto. eauto. 
con. asimpl. move/H0 : H1. asimpl. done. 
Qed. 
...
move/flattenP : H0=>[] x. move/mapP=>[]. intros. subst.
destruct x0. 
 econstructor. 
...
Inductive gInvPred3 : gType   -> Prop :=
 ...
 | rol3_gen_branch g (gs : seq (nat * gType)) l g' a :  full_unf g = GBranch a gs -> (l,g') \in gs -> gInvPred3 g' -> gInvPred3 g 
...
have : ((fun p : fin * gType => let (a0, b) := p in (a0, b [g sigma])) \o id) (l,g') \in map (fun p : fin * gType => let (a0, b) := p in (a0, b [g sigma])) gs. 
...
apply gInv_idemp2 in H. rewrite Heqn in H. inv H. 
Qed. 
...
rewrite negb_and in H1. destruct (orP H1). destruct a0.  econstructor 2. 
...
rewrite all_map. apply/allP. intro. intros. simpl. ssa. rewrite /prod_map. destruct x.  ssa. apply/H;eauto.
...
move/allP : H0. intro. apply H0 in H2. ssa. eauto. 
Qed. 
...
intros. apply/negP=> HH. apply (negP (H0 n)). destruct x.  ssa. apply/flattenP. exists (gType_fv g)=>//=.
rewrite map_snd.  apply/map_f. 
have : g = snd (n0,g). done. move=>->. apply/map_f. apply/inP.  done.
move/allP : H1.  intro. 
move/inP : xIn. move/H1. ssa. 
Qed.
